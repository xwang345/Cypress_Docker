describe('Verify drag and drop', () => {
  ...
  it('should able to drag and drop element on Material Angular example page', () => {
    ...
    // cy.step(`Drag and Drop the element to the second list`);
    cy.get('#cdk-drop-list-1 .cdk-drag').each(($el) => {
      cy.wrap($el).realMouseDown({ button: 'left', position: 'center' })
      .realMouseMove(0, 10, { position: 'center' });

      cy.wait(800) // In our case, we wait 500ms cause we have animations which we are sure that take this amount of time
      cy.get('#cdk-drop-list-2').realMouseMove(0, 0, { position: 'center' }).realMouseUp();
      cy.wait(800) // In our case, we wait 500ms cause we have animations which we are sure that take this amount of time
